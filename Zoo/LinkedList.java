public class LinkedList{
    ...
    public LinkedList duplicate(){ // creates a duplicate of the linkedlist
        LinkedList duplicateList = new LinkedList();
        Node dupNode = head;
        while(dupNode != null){ // goes though orginal node
            duplicateList.AddTail(dupNode.value); // adds refernce to same animal in new node
            dupNode = dupNode.next;
        }
        return duplicateList;
    }

    public LinkedList duplicate(Category a){ // creates a duplicate of the linkedlist with Category filter
        LinkedList duplicateList = new LinkedList();
        Node dupNode = head;
        while(dupNode != null){ // goes though orginal node
            if(dupNode.value.getCategory() == a){ // checks if category matches
                duplicateList.AddTail(dupNode.value); // adds refernce to same animal in new node
            }
            dupNode = dupNode.next;
        }
